...
## 매매 스타일 적합도 (장투·스윙·단타)
앱 상단 탭으로 전환. 각 100점 만점의 규칙 기반 참고 점수이며 매수 추천이 아님.

| 탭 | 채점 대상 | 항목 (배점) |
|---|---|---|
| 장투 | 1층 통과 | ROE(25), 부채비율(20), FCF 지속성(20), 이익의 질(15), PER(10), 매출 성장(10) |
| 스윙 | 1층 탈락 제외, 시총 1,000억+, 20일 평균 거래대금 30억+ | 추세 정렬(25), 60일선 방향(10), 20일 모멘텀(15), 20일선 눌림(15), 거래량 증가(15), ATR(10), 52주 고점 근접(10) |
| 단타 | 시장조치 종목·1,000원 미만 제외 | 20일 평균 거래대금(30), ATR(25), 당일 거래 급증(25), 당일 등락(10), 5,000원 이상(10) |

- ROE·부채비율은 주간 재무 수집에서, 가격 지표는 평일 자동 갱신의 1년 일봉 조회에서 계산
- 스윙·단타는 종가 기준 하루 1회 갱신이라 실시간 매매 신호가 아님
- 실제 매매 적용 전 과거 데이터 검증(백테스트) 권장
